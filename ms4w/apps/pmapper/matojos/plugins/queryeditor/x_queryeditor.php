...
$selectMethode = $_REQUEST['selectMethode'];
...
if ($continue) {
    ...
    if ($operation == 'getattributes') {
    	...
    	$result = array();
    	$numAttributes = min(count($attributes['valuesToUse']), count($attributes['valuesToShow']));
    	for ($iAttribute = 0 ; $iAttribute < $numAttributes ; $iAttribute++) {
    		$attribute = array();
    		$attribute['field'] = $attributes['valuesToUse'][$iAttribute];
    		$attribute['header'] = $attributes['valuesToShow'][$iAttribute];
    		$result[] = $attribute;
    	}
	...
	// Request = execute query
    	...
    	if ($mapLayers && $mapLayers[0]) {
    		...
    		foreach($mapLayers as $mapLayer) {
        		...
        		$attributes = getAttributsRealAndReadNames($layerName);
        		foreach ($attributes['valuesToUse'] as $indice => $valueToUse) {
        			$valueToShow = addslashes($attributes['valuesToShow'][$indice]);
